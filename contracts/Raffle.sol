...
contract Raffle is Ownable {
    ...
    function deleteRaffle() external onlynftOwner nftHeld vrfCalled {
        ...
        //iterates through player array and tranfers however much money they spent on tickets back to them
        for(uint256 i = players.length - 1; i >= 0; i--) {
            uint256 ticketsOwned = playerTickets[payable(players[i])];
            payable(players[i]).transfer(ticketFee * ticketsOwned);
            players.pop();
        }
    }
}
